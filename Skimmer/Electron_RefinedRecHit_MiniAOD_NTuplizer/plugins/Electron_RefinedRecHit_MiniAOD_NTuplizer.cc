...
Electron_RefinedRecHit_MiniAOD_NTuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //  clustertools_NoZS = new noZS::EcalClusterLazyTools(iEvent, iSetup, EBRechitsHandle, EERechitsHandle);
  ...
  //  const EcalRecHitCollection *recHitsEE = clustertools_NoZS->getEcalEERecHitCollection();
   ...
   for (size_t i = 0; i < electrons->size(); ++i){
      ...
      // if (refinedCluster_) {
      //    sc = ele.superCluster(); // use for refined clusters
      // } else {
      //    auto tmpCluster = ele.parentSuperCluster();

      //    if (tmpCluster.isNull() || !tmpCluster.isAvailable()) {
      //       std::cout << "Parent supercluster does not exist or is not available!" << std::endl;
      //       continue;
      //    }
      ...
      for (const auto&  detitr : hitsAndFractions) {
         if(isEB){
            ...
            const CaloCellGeometry* geom = ecalEBGeom->getGeometry(Did).get();
            ...
            const CaloCellGeometry* geom = ecalEEGeom->getGeometry(Did).get();
         ...
         auto _ped = pedestals.find(detitr.first);
         // HitNoise[nElectrons_].push_back(_ped->find(detitr.first)->rms(1));
         HitNoise[nElectrons_].push_back(_ped->rms(1));
      }  
...
void Electron_RefinedRecHit_MiniAOD_NTuplizer::GetESPlaneRecHits(const reco::SuperCluster& sc, const CaloGeometry& theGeometry, unsigned int elenum, unsigned int planeIndex) {
         ...
         for(ESRecHitCollection::const_iterator esItr = ESRechitsHandle->begin(); esItr != ESRechitsHandle->end(); ++esItr) {//loop over ES rechits to find the one in the cluster
            ...
            if(rhid == (*rh).first) { // found ESrechit
               ...
               if((int) rhid.plane() == (int) planeIndex) {
                  ...
                  const CaloCellGeometry* geom = ecalESGeom->getGeometry(rhid).get();
